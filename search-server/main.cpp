...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        if (!all_of(stop_words.begin(), stop_words.end(), [](string s) {
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (document_id < 0) {
            throw invalid_argument("Document id must be non-negative"s);
        }
        if (documents_.count(document_id)) {
            throw invalid_argument("Document with this id already exists"s);
        }
        if (!IsValidWord(document)) {
            throw invalid_argument("Document must not contain special characters"s);
        }
